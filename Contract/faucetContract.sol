...
contract faucet{
    ...
    function dotransfer(address add)  public{
         ...
         require(usertable[add].tokenaccount <=100,"you have too many token");
         if(currentTime-usertable[add].transfertime>=1 days)
         refreshtodaytoken(add);
         require(usertable[add].todaytokenaccount<3,"today is to many requst,welcome tomorrow");
        require(currentTime - usertable[add].transfertime >=5 seconds,"requesting too often");
         uint u = 1 ether;
